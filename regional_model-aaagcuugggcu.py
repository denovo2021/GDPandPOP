...
with pm.Model(coords=coords) as regional_model:
    ...
    # Expected value
    mu = alpha_region[df["region_code"].values] + beta_region[df["region_code"].values] * df["Log_Population_c"].values
...
if "Log_GDP_obs" in ppc:
    ...
    for i, reg in enumerate(regions):
        mask = df["Region"] == reg
        plt.scatter(df.loc[mask, "Log_Population_c"], pred_means[mask],
