...
with regional_model_with_quadratic:
       # ────────────────────────────────────────────────────────────────────────
    # 1. Region-level parameters – weak-informative shrinkage toward
    #    the global simple-model posterior (alpha_mean etc.).
    #    We treat the simple model’s mean as the prior mode and allow
    #    moderate dispersion around it.
    # ────────────────────────────────────────────────────────────────────────
    alpha_region = pm.Normal(
        "alpha_region",
        mu=alpha_mean,
        sigma=alpha_sd * 2.0,      # loose: 95 % ≈ ±4 × simple-model SD
        dims="Region",
    )
    beta_region = pm.Normal(
        "beta_region",
        mu=beta_mean,
        sigma=0.5,                 # same scale as refined simple model
        dims="Region",
    )
    gamma_region = pm.Normal(
        "gamma_region",
        mu=gamma_mean,
        sigma=0.1,                 # gentle curvature prior
        dims="Region",
    )

    sigma = pm.HalfStudentT("sigma", nu=3, sigma=0.3)

    # Clip ν so it stays in a stable range [2, 30]
...
x_bar = df["Log_Population_c"].mean()  
